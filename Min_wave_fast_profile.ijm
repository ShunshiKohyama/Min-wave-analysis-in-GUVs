...
// ImageJ macro for detecting GUVs and getting the Min wave profile inside GUVs for downstream analysis
...
//
// Shunshi Kohyama (The University of Tokyo), 2026
...
minArea = PI * pow((minDiam/2), 2);
maxArea =  PI * pow((maxDiam/2), 2);
run("ROI Manager...");
...
while (redo == false) {
	...
	Dialog.addChoice("Threshold method", methods, "Huang");
	Dialog.addNumber("Minimal diameter for detection (µm)", minDiam);
	Dialog.addNumber("Maximal diameter for detection (µm)", maxDiam);
...
for (i = 0; i < N; i++) {
	roiManager("select", i);
	roiManager("rename", title + "_" + i + 1);
	setResult("ID", i, Roi.getName() + ".csv");
	setResult("diameter", i, getValue("Height"));
}
setResult("scale (µm)", 0, scale);
setResult("interval (sec)", 0, interval);
setResult("channel", 0, channel);
setResult("slice", 0, slice);
setResult("frame", 0, frame);
setResult("threshold type", 0, type);
setResult("minimal diameter for detection (µm)", 0, minDiam);
setResult("maximal diameter for detection (µm)", 0, maxDiam);
setResult("minimal circularity for detection", 0, minCirc);
setResult("maximal circularity for detection", 0, maxCirc);

saveAs("Results", dir + title + "_metadata.csv");

for (i = 0; i < N; i++) {
	roiManager("select", i);
	Result = Roi.getName() + ".csv";
	run("Straighten...", "title=straight line=5 process");
	w = getValue("Width");
	run("Size...", "width=w height=1 depth=n average interpolation=Bicubic");
	run("Clear Results");
	...
	for(j = 0; j < n; j++) {
		...
		Stack.setSlice(j + 1);
		profile = getProfile();
		Table.setColumn(interval * j, profile);
		}
	saveAs("Results", dir + Result);
	run("Close", "Results");
	close();
}
